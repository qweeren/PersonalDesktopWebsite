<script>
    ...
    import GooglePage from "./GooglePage.svelte";
    ...
    openApps.subscribe((openAppsList) => {
        for (let i = 0; i < openAppsList.length; i++) {
            if(openAppsList[i][2] == 3 && HideProgram){
                HideProgram=false;
...
<div class="programcontainer" class:hidden={HideProgram}>
    <Program on:close={close} width={programWidth} height={programHeight} {appicon} appname={appname + " | Most pages won't work. I cant do anything about it."} {appid} bind:this={appref}>
        <GooglePage/>
    </Program>
</div>
